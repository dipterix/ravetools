...
recenter <- function(M, nd, v2r) {
  ctr <- v2r %*% c((nd - 1) / 2, 1)
  Tc <- diag(4); Tc[1:3, 4] <- ctr[1:3]
  Tc %*% M %*% solve(Tc)
}

rotz <- function(deg) {
  th <- deg * pi / 180; R <- diag(4)
  R[1:2, 1:2] <- matrix(c(cos(th), sin(th), -sin(th), cos(th)), 2); R
}

test_that("rigid registration recovers a known translation (all metrics)", {
  nd <- c(40L, 40L, 40L)
  v2r <- diag(4); v2r[1:3, 4] <- -20
  target <- phantom(nd)
  shift <- c(3, -2, 1)
  Ttrue <- diag(4); Ttrue[1:3, 4] <- shift
  ...
  for (m in c("meansquares", "cc", "mattes")) {
    res <- register_volume3d(
      ...
      iterations = c(200, 150, 80), sampling_rate = 0.4)
...
test_that("rigid registration recovers a known rotation", {
  skip_on_cran()
  nd <- c(48L, 48L, 48L)
  v2r <- diag(4); v2r[1:3, 4] <- -24
  target <- phantom(nd)
  Ttrue <- recenter(rotz(12), nd, v2r); Ttrue[1:3, 4] <- Ttrue[1:3, 4] + c(2, -3, 1)
  source <- apply_transform3d(target, v2r, solve(Ttrue),
                              reference_dim = nd, reference_vox2ras = v2r)
  ...
  res <- register_volume3d(
    ...
    shrink_factors = c(4, 2, 1), smoothing_sigmas = c(2, 1, 0),
    iterations = c(300, 200, 100), sampling_rate = 0.4)
...
test_that("affine registration recovers scale + shear + rotation", {
  ...
  v2r <- diag(4); v2r[1:3, 4] <- -24
  target <- phantom(nd)
  A <- diag(4); A[1, 1] <- 1.08; A[2, 2] <- 0.93; A[1, 2] <- 0.06; A[2, 3] <- -0.04
  Ttrue <- recenter(A %*% rotz(5), nd, v2r); Ttrue[1:3, 4] <- Ttrue[1:3, 4] + c(1, 1, -1)
  source <- apply_transform3d(target, v2r, solve(Ttrue),
                              reference_dim = nd, reference_vox2ras = v2r)
  ...
  res <- register_volume3d(
    ...
    shrink_factors = c(4, 2, 1), smoothing_sigmas = c(2, 1, 0),
    iterations = c(300, 200, 120), sampling_rate = 0.4)
...
test_that("registration handles different vox2ras (spacing) on source and target", {
  skip_on_cran()
  nd_t <- c(40L, 40L, 40L)
  v2r_t <- diag(4); v2r_t[1:3, 4] <- -20
  target <- phantom(nd_t)

  # source on a finer grid (0.5 mm) with an origin shift, holding the same anatomy
  nd_s <- c(80L, 80L, 80L)
  v2r_s <- diag(c(0.5, 0.5, 0.5, 1)); v2r_s[1:3, 4] <- -20
  source <- apply_transform3d(target, v2r_t, diag(4),
  ...
  res <- register_volume3d(
    ...
    shrink_factors = c(4, 2, 1), smoothing_sigmas = c(2, 1, 0),
    iterations = c(200, 150, 100), sampling_rate = 0.5)
...
test_that("SyN recovers a known smooth deformation", {
  ...
  tril <- function(vol, cx, cy, cz) {
    ...
    cx <- pmin(pmax(cx, 0), d[1]-1); cy <- pmin(pmax(cy, 0), d[2]-1); cz <- pmin(pmax(cz, 0), d[3]-1)
    x0 <- floor(cx); y0 <- floor(cy); z0 <- floor(cz)
    x1 <- pmin(x0+1, d[1]-1); y1 <- pmin(y0+1, d[2]-1); z1 <- pmin(z0+1, d[3]-1)
    fx <- cx-x0; fy <- cy-y0; fz <- cz-z0
    idx <- function(x, y, z) x + d[1]*(y + d[2]*z) + 1
    v <- function(x, y, z) vol[idx(x, y, z)]
    c00 <- v(x0,y0,z0)*(1-fx)+v(x1,y0,z0)*fx; c10 <- v(x0,y1,z0)*(1-fx)+v(x1,y1,z0)*fx
    c01 <- v(x0,y0,z1)*(1-fx)+v(x1,y0,z1)*fx; c11 <- v(x0,y1,z1)*(1-fx)+v(x1,y1,z1)*fx
    c0 <- c00*(1-fy)+c10*fy; c1 <- c01*(1-fy)+c11*fy
    out <- c0*(1-fz)+c1*fz; out[!inb] <- 0; out
  }

  g <- expand.grid(x = 0:(nd[1]-1), y = 0:(nd[2]-1), z = 0:(nd[3]-1))
  amp <- 1.4; L <- 16
  moving <- array(tril(target,
  ...
  res <- register_volume3d(
    ...
    syn_iterations = c(50, 40, 20), syn_sigma = 3)
...
test_that("metric improves over the identity transform", {
  ...
  v2r <- diag(4); v2r[1:3, 4] <- -20
  target <- phantom(nd)
  Ttrue <- recenter(rotz(8), nd, v2r); Ttrue[1:3, 4] <- Ttrue[1:3, 4] + c(2, -2, 1)
  source <- apply_transform3d(target, v2r, solve(Ttrue),
                              reference_dim = nd, reference_vox2ras = v2r)
...
test_that("masks: all-ones equals unmasked, partial mask runs, bad dims error", {
  ...
  lin <- function(tm = NULL) register_volume3d(
    source, target, v2r, v2r, target_mask = tm, type = "rigid", metric = "cc",
    shrink_factors = c(4, 2), smoothing_sigmas = c(2, 1),
    iterations = c(100, 60), sampling_rate = 1, verbose = FALSE)$transform
  ...
  m <- array(0, nd); m[6:26, 6:26, 6:26] <- 1
