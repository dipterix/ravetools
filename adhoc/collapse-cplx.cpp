...
struct CollapseComplex : public RcppParallel::Worker
{
  const SEXP x_;
  const SEXP dims_;
  ...
  void per_thread(std::size_t thread) {
    ...
    for(R_xlen_t ii = start; ii < end; ii++){
      ...
      for(kk = 0; kk < collapse_len; kk++){
        ...
        switch(method){
        case 1:
          tmp.r += x_ptr2->r * this->scale;
          tmp.i += x_ptr2->i * this->scale;
          break;
        case 2:
          tmp.r += 10.0 * this->scale * std::log10( x_ptr2->r * x_ptr2->r + x_ptr2->i * x_ptr2->i );
          break;
        case 3:
          tmp.r += (x_ptr2->r * x_ptr2->r + x_ptr2->i * x_ptr2->i) * this->scale;
          break;
        case 4:
          tmp.r += (std::sqrt( x_ptr2->r * x_ptr2->r + x_ptr2->i * x_ptr2->i )) * this->scale;
          break;
        default:
          tmp.r += x_ptr2->r * this->scale;
          tmp.i += x_ptr2->i * this->scale;
...
// [[Rcpp::export]]
SEXP collapser_cplx(SEXP x, SEXP keep,
                    const int method = 1,
                    const int average = 0) {
  SEXP re = R_NilValue;
  ...
  SEXP x_ = R_NilValue;
  if(TYPEOF(x) != CPLXSXP){
    PROTECT(x_ = Rf_coerceVector(x, CPLXSXP));
  } else {
    PROTECT(x_ = x);
  }
  ...
  if(re == R_NilValue){
    ...
    *dim_cumprod_ptr = 1;
    ...
    PROTECT(re = Rf_allocVector(CPLXSXP, re_len));
    if(nkeeps > 1){
    ...
    CollapseComplex collapse(x_, dims_, dim_cumprod, keep_, remain, re, dim_re,
