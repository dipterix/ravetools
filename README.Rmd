...
Installing the package from source requires installation of proper compilers and some `C` libraries; see [this document](https://github.com/dipterix/ravetools/blob/master/installation.md) for details.
...
`ravetools` provides imaging co-registration via [`NiftyReg`](https://doi.org/10.1117/1.JMI.1.2.024003). You can align `CT` to `MRI`, or `MRI` (T2) to `MRI` (T1). The method can be body `rigid`, `affine`, or `non-linear`.
