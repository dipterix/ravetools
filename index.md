...
| Part | Contents |
|:---|:---|
| [1. Signal processing](#id_1-signal-processing) | Channel diagnosis and time-frequency decomposition; brain stimulation tools; filter design |
| [2. Image processing](#id_2-image-processing) | 3D volume registration and resampling; surface reconstruction and rendering; low-level geometry |
| [3. Utilities and helpers](#id_3-utilities-and-helpers) | `FFTW3` bindings, parallel array operations, geometry classes, binary readers |
...
![](https://github.com/dipterix/ravetools/blob/master/adhoc/README-figures/ccep-1.png?raw=true)
...
![](https://github.com/dipterix/ravetools/blob/master/adhoc/README-figures/filter-design-1.png?raw=true)
...
![](https://github.com/dipterix/ravetools/blob/master/adhoc/README-figures/registration-1.png?raw=true)
...
`ravetools` carries the `ravetools_mesh3d` class, so
[`plot()`](https://rdrr.io/r/graphics/plot.default.html) renders it
...
![](https://github.com/dipterix/ravetools/blob/master/adhoc/README-figures/surface-1.png?raw=true)
...
whichever suits the mesh. If `rgl` is available, `rgl_view`, `rgl_call`,
and `rgl_plot_normals` drive it instead.
...
They compose. Two primitives, the three `Query` functions asked about
them, and one `plot` call:

``` r

# A unit sphere, and a plane tilted so that it slices through it
...
plot_mesh_polygon(
  ...
  main = "Distance to the sphere; cut faces in red"
)
plot_mesh_polygon(
  list(sphere, pierce),
  ...
  shadow_color = "white", ambient_intensity = 0.55,
  main = "Where a downward ray enters the sphere"
)
par(oldpar)
```

![](https://github.com/dipterix/ravetools/blob/master/adhoc/README-figures/geometry-1.png?raw=true)

307 of the plane’s triangles cut the sphere, and 780 of the 1600 dropped
rays land on it. The two distances answer different questions:
`vcg_detect_collision` reports the exact minimum distance to the
surface, while `vcg_kdtree_nearest` reports the distance to the nearest
*vertex* - the sampled approximation that is cheaper and usually good
enough. A point cloud handed to `plot_mesh_polygon` with no face matrix
is drawn as one small sphere per point, which is how the ray hits are
rendered above.

The
[`vignette("mesh-geometry", package = "ravetools")`](https://dipterix.org/ravetools/articles/mesh-geometry.md)
article walks the rest of the table - repair, re-meshing, smoothing,
inflation, patches, and geodesic paths - on a real hippocampus surface.
